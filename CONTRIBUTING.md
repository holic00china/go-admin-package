...
See [manual](https://go-admin-package/projects/3) for more information.
...
For now, you need to add your fork as a remote on the original **\$GOPATH**/src/go-admin-package clone, so:

```bash

$ go get go-admin-package
$ cd $GOPATH/src/go-admin-package # GOPATH is $HOME/go by default.
...
Notice: `go get` return `package go-admin-package: no Go files in /go/src/go-admin-package` is normal.
...
* **Support on [Open Collective](https://opencollective.com/go-admin-package)**
...
* [Discussions](http://forum.go-admin-package.cn)
* [Tasks](https://go-admin-package/projects)
