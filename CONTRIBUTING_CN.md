...
在这里：[功能规划](https://go-admin-package/projects/3) 可以获得更多信息。
...
然后，你需要fork远程的master分支到你本地 **\$GOPATH**/src/go-admin-package ：
...
注意了： `go get` 返回 `package go-admin-package: no Go files in /go/src/go-admin-package` 是正常的。
